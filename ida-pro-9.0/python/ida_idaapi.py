...
class __IDAPython_Completion_Util(object):
    ...
    def __render_docstr(self, doc, name, proto):
        import ida_lines as il
        h = lambda s, c: f"{il.SCOLOR_ON}{c}{s}{il.SCOLOR_OFF}{c}"
        ha = lambda strs, cs: "".join([ h(s, c) for s, c in zip(strs, cs) ])
...
# proto = (args, types, defaults, rets)
        ...
        for l in doc.splitlines():
            ...
            if len(l) == 0:
                continue
            elif l.startswith(name) and "(" in l and ")" in l or ") -> " in l:
...
# python keywords (for example from -> _from)
    ...
    def __proto_from_docstring(self, name, doc, altname = None):
        ...
        if altname is not None:
            name = altname
        for l in doc.splitlines():
            if l.startswith(name) and "(" in l and ")" in l:
...
# argument types from @param docstring
                m = re.match(r"@param ([^:]*):", l)
                if not m:
                    continue
                arg = m[1].lstrip("_")
                typ = ":".join(l.split(":")[1:])
                if "C++:" in typ:
                    typ = re.match(r".*\(C\+\+: (.*)\).*", typ)[1]
                else:
                    typ = typ.strip(" ")

                types[args.index(arg)] = typ
...
# get low-level swig auto generated function
                        ...
                        out.append((name, hint, doc, [ W_CMEMB ]))
                    else:
...
# insert one entry per prototype
                        ...
                        for prot in prots:
                            docr = self.__render_docstr(doc, name, prot)
                            hint = self.__render_proto(name, *prot)
                            out.append((name, hint, docr, [ W_FUNC ]))
                    else:
...
# constructor
                    doc = inspect.getdoc(attr)
                    ls = inspect.getdoc(getattr(attr, "__init__")).splitlines()
                    for l in ls:
                        if not l.startswith("__init__"):
# NOTE: No else. If constructor doesn't have a
# documentation, assume it's a class that should
# not be instantiated directly and we silently
# remove it from the auto completion as a whole.
                            continue

                        l = l.replace("__init__", f"{name}")
                        prot = self.__proto_from_docstring(name, l)[0]
                        hint = self.__render_proto(name, *prot, is_ctor = True)
                        out.append((name, hint, doc, [ W_CTOR ]))
...
def format_basestring(_in: "PyObject *") -> "PyObject *":
    r"""
...
def pygc_set_groups_visibility(_self: "PyObject *", groups: "PyObject *", expand: "PyObject *", new_current: "PyObject *") -> "PyObject *":
    ...
    @param new_current: PyObject *
    """
    return _ida_idaapi.pygc_set_groups_visibility(_self, groups, expand, new_current)
...
class loader_input_t(object):
    ...
    __repr__ = _swig_repr
    __idc_cvt_id__: "int" = property(_ida_idaapi.loader_input_t___idc_cvt_id___get, _ida_idaapi.loader_input_t___idc_cvt_id___set, doc=r"""__idc_cvt_id__""")
    ...
    def __init__(self, pycapsule: "PyObject *"=None):
        ...
        """
        _ida_idaapi.loader_input_t_swiginit(self, _ida_idaapi.new_loader_input_t(pycapsule))
    ...
    def open(self, filename: "char const *", remote: "bool"=False) -> "bool":
        r"""
    ...
    def open_memory(self, start: "ea_t", size: "asize_t"=0) -> "bool":
        r"""
    ...
    def getz(self, sz: "size_t", fpos: "int64"=-1) -> "PyObject *":
        r"""
        ...
        """
        return _ida_idaapi.loader_input_t_getz(self, sz, fpos)

    def gets(self, len: "size_t") -> "PyObject *":
        r"""
    ...
    def file2base(self, pos: "int64", ea1: "ea_t", ea2: "ea_t", patchable: "int") -> "int":
        ...
        """
        return _ida_idaapi.loader_input_t_file2base(self, pos, ea1, ea2, patchable)
